...
`))}return u.join(`
`)}function Ns(t,e){if(!t||t.length===0)return"";let u=gs(t),n=u.length;if(n===0)return"";let{stdoutColumns:r,columns:s}=Es(e);if(s.length>n)throw new Error(`${s.length} columns defined, but only ${n} columns found`);let i=ys(r,s,u);return t.map(D=>Os(i,D)).join(`
`)}U();var Hs=["<",">","=",">=","<="];function Ps(t){if(!Hs.includes(t))throw new TypeError(`Invalid breakpoint operator: ${t}`)}function Ls(t){let e=Object.keys(t).map(u=>{let[n,r]=u.split(" ");Ps(n);let s=Number.parseInt(r,10);if(Number.isNaN(s))throw new TypeError(`Invalid breakpoint value: ${r}`);let i=t[u];return{operator:n,breakpoint:s,value:i}}).sort((u,n)=>n.breakpoint-u.breakpoint);return u=>{var n;return(n=e.find(({operator:r,breakpoint:s})=>r==="="&&u===s||r===">"&&u>s||r==="<"&&u<s||r===">="&&u>=s||r==="<="&&u<=s))==null?void 0:n.value}}const Is=t=>t.replace(/[-_ ](\w)/g,(e,u)=>u.toUpperCase()),ks=t=>t.replace(/\B([A-Z])/g,"-$1").toLowerCase(),Ms={"> 80":[{width:"content-width",paddingLeft:2,paddingRight:8},{width:"auto"}],"> 40":[{width:"auto",paddingLeft:2,paddingRight:8,preprocess:t=>t.trim()},{width:"100%",paddingLeft:2,paddingBottom:1}],"> 0":{stdoutColumns:1e3,columns:[{width:"content-width",paddingLeft:2,paddingRight:8},{width:"content-width"}]}};function Ws(t){let e=!1;return{type:"table",data:{tableData:Object.keys(t).sort((n,r)=>n.localeCompare(r)).map(n=>{const r=t[n],s="alias"in r;return s&&(e=!0),{name:n,flag:r,flagFormatted:`--${ks(n)}`,aliasesEnabled:e,aliasFormatted:s?`-${r.alias}`:void 0}}).map(n=>(n.aliasesEnabled=e,[{type:"flagName",data:n},{type:"flagDescription",data:n}])),tableBreakpoints:Ms}}}const Mu=t=>{var e;return!t||((e=t.version)!=null?e:t.help?t.help.version:void 0)},Wu=t=>{var u;const e="parent"in t&&((u=t.parent)==null?void 0:u.name);return(e?`${e} `:"")+t.name};function Gs(t){var n;const e=[];t.name&&e.push(Wu(t));const u=(n=Mu(t))!=null?n:"parent"in t&&Mu(t.parent);if(u&&e.push(`v${u}`),e.length!==0)return{id:"name",type:"text",data:`${e.join(" ")}
`}}function js(t){const{help:e}=t;if(!(!e||!e.description))return{id:"description",type:"text",data:`${e.description}
`}}function Us(t){var u;const e=t.help||{};if("usage"in e)return e.usage?{id:"usage",type:"section",data:{title:"Usage:",body:Array.isArray(e.usage)?e.usage.join(`
`):e.usage}}:void 0;if(t.name){const n=[],r=[Wu(t)];if(t.flags&&Object.keys(t.flags).length>0&&r.push("[flags...]"),t.parameters&&t.parameters.length>0){const{parameters:s}=t,i=s.indexOf("--"),D=i>-1&&s.slice(i+1).some(o=>o.startsWith("<"));r.push(s.map(o=>o!=="--"?o:D?"--":"[--]").join(" "))}if(r.length>1&&n.push(r.join(" ")),"commands"in t&&((u=t.commands)==null?void 0:u.length)&&n.push(`${t.name} <command>`),n.length>0)return{id:"usage",type:"section",data:{title:"Usage:",body:n.join(`
`)}}}}function Ks(t){var u;return!("commands"in t)||!((u=t.commands)!=null&&u.length)?void 0:{id:"commands",type:"section",data:{title:"Commands:",body:{type:"table",data:{tableData:t.commands.map(n=>[n.options.name,n.options.help?n.options.help.description:""]),tableOptions:[{width:"content-width",paddingLeft:2,paddingRight:8}]}},indentBody:0}}}function Vs(t){if(!(!t.flags||Object.keys(t.flags).length===0))return{id:"flags",type:"section",data:{title:"Flags:",body:Ws(t.flags),indentBody:0}}}function zs(t){const{help:e}=t;if(!e||!e.examples||e.examples.length===0)return;let{examples:u}=e;if(Array.isArray(u)&&(u=u.join(`
...
`));const e=rn(Fr(yr),{noCache:Boolean(t.flags.noCache),tsconfigPath:t.flags.tsconfig}),u=async n=>{await Promise.race([new Promise(s=>{function i(D){D&&D.type==="kill"&&(s(D.signal),e.off("message",i))}e.on("message",i)}),new Promise(s=>{setTimeout(s,10)})])||e.kill(n)};process.on("SIGINT",u),process.on("SIGTERM",u),e.on("close",n=>process.exit(n))});
